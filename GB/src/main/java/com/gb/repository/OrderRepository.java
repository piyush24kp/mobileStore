...
public class OrderRepository {
	...
	public Integer createOrder(OrderDetail odDetailVo) {
		String baseQuery = "INSERT INTO orderDetails(orderId, orderName,quantity,brand,category,purchasePrice,sellPrice,suppliedBy,orderDate,model) VALUES(?,?,?,?,?,?,?,?,?,?)";
		Object[] params = new Object[] { odDetailVo.getOrderId(), odDetailVo.getOrderName(), odDetailVo.getQuantity(),
				odDetailVo.getBrand(), odDetailVo.getCategory(), odDetailVo.getPurchasePrice(),
	...
	public Integer updateOrder(OrderDetail odDetailVo) {
		String baseQuery = "UPDATE orderDetails set orderId = ?, orderName = ?,quantity = ?,brand = ?,category = ?,purchasePrice = ?,sellPrice = ?,suppliedBy = ?,orderDate = ?,model = ? WHERE orderId = ?";
		Object[] params = new Object[] { odDetailVo.getOrderId(), odDetailVo.getOrderName(), odDetailVo.getQuantity(),
				odDetailVo.getBrand(), odDetailVo.getCategory(), odDetailVo.getPurchasePrice(),
	...
	public List<ModelDetail> getModelByBrandId(Long brandId) {
		List<ModelDetail> brandDetail = new ArrayList();
		String baseQuery = "select * FROM models WHERE brandId = " + brandId;
		brandDetail = jdbcTemplate.query(baseQuery, new BeanPropertyRowMapper(ModelDetail.class));
		return brandDetail;
	}

	public ModelDetail getModelById(Long modelId) {
	...
	public Integer setModel(ModelDetail modelDetail) {
		String baseQuery = "INSERT INTO models(modelName,brandId,storage,price) VALUES(?,?,?,?)";
		Object[] params = new Object[] { modelDetail.getModelName(),modelDetail.getBrandId(),modelDetail.getStorage(),modelDetail.getPrice() };
		return jdbcTemplate.update(baseQuery, params);
	}

	public ModelDetail getUpdateModel(ModelDetail modelDetail) {
		List<ModelDetail> details = new ArrayList();
